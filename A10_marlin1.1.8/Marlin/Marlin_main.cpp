 ...
 */
static bool drain_injected_commands_P() {//ÅÐ¶ÏÁ¢¼´ÃüÁî
 ...
 */
void enqueue_and_echo_commands_P(const char * const pgcode) {//Ìí¼ÓÁ¢¼´ÃüÁî
...
void get_available_commands() {
  ...
  if (drain_injected_commands_P()) return;//ÅÐ¶ÏÓÐÎÞÁ¢¼´ÃüÁî
...
void idle(           //±£³Ö»úÆ÷»îÁ¦
  ...
  lcd_update();   //Ë¢ÐÂLCDÏÔÊ¾

  host_keepalive();  //¶¨Ê±Êä³öÃ¦ÐÅÏ¢£¬»úÆ÷²»½ÓÊÜÃüÁî

  #if ENABLED(AUTO_REPORT_TEMPERATURES) && (HAS_TEMP_HOTEND || HAS_TEMP_BED)
    thermalManager.auto_report_temperatures();//×Ô¶¯ÉÏ´«ÎÂ¶ÈÐÅÏ¢
  #endif

  manage_inactivity(  //´¦Àí´òÓ¡¸¨Öú¹¦ÄÜ
    #if ENABLED(ADVANCED_PAUSE_FEATURE)
      no_stepper_sleep
    #endif
  );

  thermalManager.manage_heater();  //¼ÓÈÈ´¦Àí
