...
void Endstops::M119() {
  ...
  #define ES_REPORT(AXIS) do{ \
    ...
    if((AXIS == Z_MIN) && bltouch_endstop_flag) \
		SERIAL_PROTOCOLLN(((READ(AXIS##_PIN)^ bltouch_endstop_flag) ? MSG_ENDSTOP_HIT : MSG_ENDSTOP_OPEN)); \
	else \
...
void Endstops::update() {
  ...
  // UPDATE_ZENDSTOP_BIT: set the current endstop bits for an endstop to its status
  #define ZUPDATE_ENDSTOP_BIT(AXIS, MINMAX) SET_BIT(current_endstop_bits, _ENDSTOP(AXIS, MINMAX), (READ(_ENDSTOP_PIN(AXIS, MINMAX)) != bltouch_endstop_flag))
  ...
  #define ZUPDATE_ENDSTOP(AXIS,MINMAX) do { \
  	  ZUPDATE_ENDSTOP_BIT(AXIS, MINMAX); \
      if (TEST_ENDSTOP(_ENDSTOP(AXIS, MINMAX)) && stepper.current_block->steps[_AXIS(AXIS)] > 0) { \
        _ENDSTOP_HIT(AXIS, MINMAX); \
        stepper.endstop_triggered(_AXIS(AXIS)); \
      } \
  ...
  if (Z_MOVE_TEST) {
    ...
    if (stepper.motor_direction(Z_AXIS_HEAD)) { // Z -direction. Gantry down, bed up.
	  ...
	  //#error 333
		  ...
		  //#error 330
            if (z_probe_enabled) {
				//SERIAL_PROTOCOLLNPGM("blZ");
				UPDATE_ENDSTOP(Z, MIN);
            }
			if (level_sensor_enabled == false)
			{
				//SERIAL_PROTOCOLLNPGM("Z");
				ZUPDATE_ENDSTOP(Z, MIN);
			}
          #else
		  #error 331
            UPDATE_ENDSTOP(Z, MIN);
