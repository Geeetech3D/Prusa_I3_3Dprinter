...
static void lcd_implementation_status_screen() {//liu...
  ...
  if (PAGE_UNDER(STATUS_SCREENHEIGHT + 1)) {
	...
	u8g.drawBitmapP(9, 1, (STATUS_SCREENWIDTH + 7) / 8, STATUS_SCREENHEIGHT,
      #if HAS_FAN0
        blink && fanSpeeds[0] ? status_screen01_bmp : status_screen11_bmp
      #else
        status_screen01_bmp
      #endif
    );
    ...
    u8g.drawBitmapP(9, 1, (STATUS_SCREENWIDTH + 7) / 8, STATUS_SCREENHEIGHT,
      #if HAS_FAN0
        blink && fanSpeeds[0] ? status_screen00_bmp : status_screen10_bmp
      #else
        status_screen00_bmp
      #endif
    );
  ...
  if (PAGE_CONTAINS(XYZ_FRAME_TOP, XYZ_FRAME_TOP + XYZ_FRAME_HEIGHT - 1)) {
    ...
    if (PAGE_CONTAINS(XYZ_BASELINE - (INFO_FONT_HEIGHT - 1), XYZ_BASELINE)) {
      ...
      */
	  ...
	  const int mix_pct1 = int(RECIPROCAL(mixing_factor[NOZZLE1]) * 100.0f);
      const char * const fmt = mixer.gradient_flag ? PSTR("Mx%d;%d;%d%% ") : PSTR("Mx%d;%d;%d%% ");
      sprintf_P(mixer_messages, fmt, mix_pct, mix_pct1, 100 - mix_pct - mix_pct1);
