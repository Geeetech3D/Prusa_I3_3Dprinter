...
ISR(TIMER1_COMPA_vect) {
  ...
  if (filament_runout_enabled) {
    if ( (READ(FIL_RUNOUT_PIN) || READ(FIL_RUNOUT2_PIN) || READ(FIL_RUNOUT3_PIN))
    ...
    if (test && !READ(FIL_RUNOUT_PIN) && !READ(FIL_RUNOUT2_PIN) && !READ(FIL_RUNOUT3_PIN)) {
  	...
  	if(msg_ready)
  	{
		...
		msg_ready = false;
		test = false;
  		//SERIAL_ECHOLN("filament disable");
  ...
  //
    ...
    if (!READ(CONTINUITY_PIN) && powerloss.P_file_name[0] && powerloss.recovery == Rec_Idle && (print_job_timer.isRunning() || print_job_timer.isPaused())) {
