...
#if INTEGRATED_FW
bool level_sensor_enabled = false;
bool bltouch_endstop_flag = true;
#endif
...
static void homeaxis(const AxisEnum axis) {
  ...
  bump_t = axis_home_dir * (
    #if HOMING_Z_WITH_PROBE_V
      (axis == Z_AXIS) ? max(Z_CLEARANCE_BETWEEN_PROBES, home_bump_mm(Z_AXIS)) :
    #endif
    home_bump_mm(axis)
  );
  }
  const float bump = bump_t;
  #endif
...
inline void gcode_M355() {
  ...
  inline void gcode_M163() {
	...
	if(mix_index == 1)
	{
		powerloss.Nozzle1_Value = (uint8_t)(mix_value*100);
	}
    }
  }
...
void reset_mix_ratios() {
   // Virtual Tools 0, 1, 2, 3 = Filament 1, 2, 3, 4, etc.
	...
	mixing_factor[NOZZLE0] = RECIPROCAL(1);
}
  ...
  lcd_init();
    ...
    for (uint8_t i = 0; i < MIXING_STEPPERS; i++)
		...
		if (powerloss.recovery != Rec_Outage) {
			 mixing_factor[NOZZLE0] = RECIPROCAL(1);
			 powerloss.Nozzle0_Value = (uint8_t)(100);
			 powerloss.Nozzle1_Value = (uint8_t)(0);
		}
  #endif
...
if(level_sensor_enabled == true)
{
  ...
  if (powerloss.recovery == Rec_Outage) {
    ...
    //if (powerloss.recovery == Rec_Idle) {
...
//PATCH if A20 Gcode is run
  //if((mixing_factor[NOZZLE0] && mixing_factor[NOZZLE1] && mixing_factor[NOZZLE2]) == 0)
  //{
	//	mixing_factor[NOZZLE0] = RECIPROCAL(1);
  //}
  ...
  if (commands_in_queue == 0) {
	...
	if(powerloss.Nozzle0_Value <101)
	{
		...
		mixing_factor[NOZZLE1] = RECIPROCAL( powerloss.Nozzle1_Value* 0.01);
		mixing_factor[NOZZLE2] = RECIPROCAL((100-powerloss.Nozzle0_Value-powerloss.Nozzle1_Value) * 0.01);
