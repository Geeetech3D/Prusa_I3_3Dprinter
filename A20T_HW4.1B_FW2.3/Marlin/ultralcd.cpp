...
void kill_screen(const char* lcd_msg) {
  ...
  inline void line_to_z(const float &z) {
    ...
    void lcd_mixer_mix_edit() {
      if (encoderPosition != 0) {
        ...
        mixer.rate[NOZZLE1] = (100 - mixer.rate[NOZZLE0])/2;
		mixer.rate[NOZZLE2] = (100 - mixer.rate[NOZZLE0])/2;
        
      }
      char tmp[21];
      
      sprintf_P(tmp, PSTR(MSG_MIX ":%3d%%;%3d%%;%3d%%"), mixer.rate[NOZZLE0], mixer.rate[NOZZLE1], mixer.rate[NOZZLE2]);
	 ...
	 powerloss.Nozzle1_Value = mixer.rate[NOZZLE1];
        _lcd_mixer_update_mix();
        lcd_goto_previous_menu();
      }
    }
    ...
    void lcd_mixer_menu() {
     ...
     mixer.rate[NOZZLE1] = int16_t(RECIPROCAL(mixing_factor[NOZZLE1]) * 100.0);
	 mixer.rate[NOZZLE2] = 100 - mixer.rate[NOZZLE0] - mixer.rate[NOZZLE1];
 
    
      char tmp[15];
      MENU_ITEM(submenu, MSG_MIX, lcd_mixer_mix_edit);
      MENU_ITEM_ADDON_START(4);
 #if 1       
        sprintf_P(tmp, PSTR(":%3d;%3d;%3d%%"), mixer.rate[NOZZLE0], mixer.rate[NOZZLE1], mixer.rate[NOZZLE2]);
  #endif
 ...
 /*#if ENABLED(LEVEL_BED_CORNERS) && DISABLED(LCD_BED_LEVELING)
  ...
  #else //End LCD GEEETECH LCD MENU
  ...
  void lcd_menu_leveling()
  {
	  ...
	  MENU_BACK(MSG_PREPARE);
		   ...
		   if (axis_homed[X_AXIS] && axis_homed[Y_AXIS] && axis_homed[Z_AXIS])
			   MENU_ITEM(function, MSG_LEVEL_CORNERS, _lcd_level_bed_corners);
 	 #endif
	  //MENU_ITEM_EDIT_CALLBACK(bool, MSG_BLTOUCH_OPT, (bool*)&level_sensor_enabled, lcd_store_settings);
	  //STATIC_ITEM("" MSG_CAP_LEVELING, true);
	  END_MENU();
  
  }
  
	void lcd_cap_leveling()
	{
		START_MENU();
		MENU_BACK(MSG_PREPARE);
		//
	...
	void lcd_level_sensor_switching()
	{
		START_MENU();
		MENU_BACK(MSG_PREPARE);
		//
	...
	void lcd_leveling_menu()
	{
	  ...
	  //MENU_ITEM(submenu, MSG_LEVEL_SENSOR_SWITCH, lcd_level_sensor_switching);
	  MENU_ITEM_EDIT_CALLBACK(bool, MSG_BLTOUCH_OPT, (bool*)&level_sensor_enabled, lcd_store_settings);
  
	  //MENU_ITEM(submenu, MSG_MENU_LEVELING, lcd_menu_leveling);
	  #if ENABLED(LEVEL_BED_CORNERS) && DISABLED(LCD_BED_LEVELING)
		   if (axis_homed[X_AXIS] && axis_homed[Y_AXIS] && axis_homed[Z_AXIS])
			   MENU_ITEM(function, MSG_LEVEL_CORNERS, _lcd_level_bed_corners);
 	 	#endif
			...
			#endif
    #endif
	  //MENU_ITEM(submenu, MSG_CAP_LEVELING, lcd_cap_leveling);  
  ...
  void lcd_prepare_menu() {
		...
		// Levelling
		//
		MENU_ITEM(submenu, MSG_LEVELING, lcd_leveling_menu);
   ...
   */
    ...
    void lcd_info_menu() {
	  ...
	  STATIC_ITEM(MSG_INFO_BAUDRATE ": " STRINGIFY(BAUDRATE), true); // Baud: 250000
		  // SERIAL_ECHOPAIR("hardware version:", hardware_version);  //liu..
			//STATIC_ITEM(MSG_FW_VER, false, true);
	  STATIC_ITEM("" MSG_HW_VER, true);//MSG_HW_VER liu
	  STATIC_ITEM("" MSG_FW_VER, true);
      END_MENU();
    }
