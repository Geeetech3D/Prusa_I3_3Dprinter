...
void Planner::_buffer_steps(const int32_t (&target)[XYZE], float fr_mm_s, const uint8_t extruder) {
  ...
  // For a mixing extruder, get a magnified step_event_count for each [liu]
    ...
    //for (uint8_t i = 0; i < MIXING_STEPPERS; i++)
 	...
 	for (uint8_t i = 0; i < MIXING_STEPPERS; i++)
    {
    	//if(((mixing_factor[i]*100) > 0) && ((mixing_factor[i]*100) > 2000)) {//Set default to 9%(11.11)/5%(20) if set between 8%/4% to 1% using LCD (Workout to fix Filament blocking)
      		//block->mix_event_count[i] = 20 * block->step_event_count;
			//SERIAL_ECHOLNPAIR("Less then 5 above 0:",block->mix_event_count[i]);
    	//}else if(((mixing_factor[i]*100) > 100) && ((mixing_factor[i]*100) < 105)){ //Set default to 91%(1.10)/95%(1.05) if set between 92%/95% to 99% using LCD (Workout to fix Filament blocking)
    		//block->mix_event_count[i] = 1.05 * block->step_event_count;
			//SERIAL_ECHOLNPAIR("Less then 95 to 100:",block->mix_event_count[i]);
		//}else {
			block->mix_event_count[i] = mixing_factor[i] * block->step_event_count; //All values with LCD is valid
			//SERIAL_ECHOLNPAIR("0 or 100 or other:",block->mix_event_count[i]);
			//SERIAL_ECHOLNPAIR("mixing factor:",mixing_factor[i]);
			//SERIAL_ECHOLNPAIR("mixer factor:",mixer.rate[i]);
		//}
	}     
