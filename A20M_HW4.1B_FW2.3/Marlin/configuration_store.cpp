...
#define EEPROM_VERSION "V402"    //"V47"  M20M
...
float hardware_version=0.1;
  ...
  bool MarlinSettings::poweroff_save() {
	...
	//3D touch enable or disable
	EEPROM_WRITE(level_sensor_enabled);
  ...
  bool MarlinSettings::poweroff_load() {
	...
	//3D touch enable or disable
	EEPROM_READ(level_sensor_enabled);
	if(level_sensor_enabled != false)
		level_sensor_enabled = true;

	if(level_sensor_enabled == true) {
		bltouch_endstop_flag = false;
	} else {
		//#undef Z1_MIN_ENDSTOP_INVERTING
        //#define Z1_MIN_ENDSTOP_INVERTING true
		bltouch_endstop_flag = true;
	}
