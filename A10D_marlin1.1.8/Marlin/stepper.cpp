...
ISR(TIMER1_COMPA_vect) {
     ...
     if(filament_switch == true){
		if(((digitalRead(A12)==1 ||digitalRead(A13)==1))&&(((P_file_name[0])&&(recovery==0)&&(print_job_timer.isRunning()==true))||(test!=1)))
		{
			test=1;
		...
		if(test==1)
		{
			if(digitalRead(A12)==0&&digitalRead(A13)==0)
			{
     ...
     if((digitalRead(A15)==0)&&(P_file_name[0])&&(recovery==0)&&(print_job_timer.isRunning()==true))// power off
	  {
	   ...
	   T1_t=thermalManager.degTargetHotend(1)+0.5;
	  ...
	  recovery=0;
	   ...
	   sprintf_P(tmp_d,PSTR("Z%u,E%lu,P%lu,T%u,T%u,,B%u,"),Z_t,E_t,pos_t,T0_t,T1_t,B_t);
	   SERIAL_ECHOLN(tmp_d);
